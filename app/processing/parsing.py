    ...
    for entity_id, entity_data in entities.items():
        try:
            serialized[str(entity_id)] = {
                ...
                "relationships": serialize_context(
...
def enrich_chunk_metadata(chunk_data, value):
    ...
    if isinstance(value, (int, float)):
        ...
        chunk_data["numeric_metadata"] = {
            "value_exact": val,
            "value_range": {"min": val - abs(val * 0.1), "max": val + abs(val * 0.1)},
        ...
        if isinstance(node, dict):
            ...
            for key, entity_type in entity_types.items():
                if key in node:
                    ...
                    found_entity["context"] = {
                        k: v for k, v in node.items() if k != key
                    }
                    break
...
def json_to_path_chunks(
    ...
    def process_value(obj, path="", context=None):
        ...
        if isinstance(obj, dict):
            ...
            chunk = {"path": path, "value": obj, "context": context}
            if entities:
...
def process_json_document(
    json_obj: Dict, file_path: str = "", max_chunks: int = 100
...
def extract_context(json_obj, path, max_depth=3):
    ...
    current = "$"
    context[current] = (
        str(json_obj)[:100] + "..." if len(str(json_obj)) > 100 else str(json_obj)
    )
