...
def upsert_file_metadata(
    conn, filename: str, file_hash: str, mod_time: datetime
) -> None:
    """
...
def get_files_to_process(
    conn, compute_file_hash, get_json_files
...
def reset_database(conn) -> None:
    ...
    cur = conn.cursor()
    try:
        # Check if tables exist before truncating them
        cur.execute("""
            SELECT table_name 
...
def init_db(conn) -> None:
    ...
    CREATE TABLE IF NOT EXISTS chunk_key_values (
        id SERIAL PRIMARY KEY,
        chunk_id TEXT REFERENCES json_chunks(id) ON DELETE CASCADE,
        key_name TEXT,
        value_text TEXT,
        data_type TEXT,
        path TEXT,
        CONSTRAINT unique_chunk_key UNIQUE (chunk_id, key_name, path)
    )
    """)
    ...
    CREATE TABLE IF NOT EXISTS chunk_archetypes (
        ...
        CONSTRAINT unique_chunk_archetype UNIQUE (chunk_id, archetype)
    )
    """)
    ...
    CREATE TABLE IF NOT EXISTS chunk_relationships (
        id SERIAL PRIMARY KEY,
        source_chunk TEXT REFERENCES json_chunks(id) ON DELETE CASCADE,
        target_chunk TEXT REFERENCES json_chunks(id) ON DELETE CASCADE,
        relationship_type TEXT,
        metadata JSONB,
        detected_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
        CONSTRAINT unique_relationship UNIQUE (source_chunk, target_chunk, relationship_type)
    )
    """)

    # Create JSON summaries table
    cur.execute("""
    ...
    # Create indexes
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_summaries_group_type ON json_summaries(group_type)"
    )
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_summaries_chunks ON json_summaries USING GIN(chunk_ids)"
    )
    cur.execute(
        "CREATE INDEX IF NOT EXISTS idx_chunks_group ON json_chunks(group_key, group_type)"
    )

    conn.commit()


def save_chunk_archetypes(
    conn, chunk_id: str, archetypes: List[Tuple[str, float]]
) -> None:
    """
...
def save_chunk_relationships(conn, relationships: List[Dict]) -> None:
    ...
    try:
        for rel in relationships:
            ...
            cur.execute(
                query, (rel["source"], rel["target"], rel["type"], json.dumps(metadata))
            )
        conn.commit()
    except Exception as e:
        conn.rollback()
...
def store_chunk(
    ...
    with conn.cursor() as cur:
        ...
        RETURNING id
        """,
            (chunk_id, json.dumps(chunk), timestamp, group_key, group_type),
        )
...
def get_parent_chunks(conn, chunk_id: str, max_depth: int = 3) -> List[Dict]:
    ...
    while depth < max_depth:
        ...
        parents.append(
            {"id": parent_id, "content": json.loads(chunk_json), "metadata": metadata}
        )
...
def get_chunk_with_context(
    conn, chunk_id: str, max_parent_depth: int = 3, max_child_depth: int = 2
) -> Dict:
    """
