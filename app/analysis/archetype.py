...
class ArchetypeDetector:
    ...
    def detect_dataset_patterns(self, structures: List[Dict]) -> Dict:
        ...
        return {"archetypes": archetype_results, "relationships": relationships}
    ...
    def _detect_record_pattern(self) -> Dict:
        ...
        has_stable_values = any(
            len(values) == 1 for values in self.value_patterns.values()
        )

        confidence = len(found) / len(record_indicators)
        if has_identity and has_description:
            confidence *= (
                2.0  # Boost confidence if we have both identity and description
            )
    ...
    def _detect_event_pattern(self) -> Dict:
        ...
        has_status = bool(
            self.value_patterns.get("status") or self.value_patterns.get("state")
        )
        ...
        return {
            "confidence": confidence,
            "common_fields": list(
                found | self.temporal_fields
            ),  # Include detected time fields
    ...
    def _detect_reference_pattern(self) -> Dict:
        ...
        reference_indicators = [
            f
            for f in self.field_frequencies.keys()
    ...
    def _detect_metric_pattern(self) -> Dict:
        ...
        if has_numbers and has_quantities:
            confidence *= (
                2.0  # Boost confidence if we have both numbers and quantity fields
            )
        elif has_numbers:
        ...
        return {
            "confidence": confidence,
            "common_fields": list(
                found | self.numerical_fields
            ),  # Include detected number fields
    ...
    def _score_event_log(self, data: Dict) -> float:
        ...
        for key, value in data.items():
            if any(
                re.search(pattern, str(value)) for pattern in self.timestamp_patterns
            ):
                has_timestamp = True
                score += 0.4
                break
    ...
    def _score_metric_data(self, data: Dict) -> float:
        ...
        numerical_values = sum(1 for v in data.values() if isinstance(v, (int, float)))
    ...
    def _score_api_response(self, data: Dict) -> float:
        ...
        for value in data.values():
            if isinstance(value, (int, str)) and any(
                re.match(pattern, str(value)) for pattern in self.http_status_patterns
            ):
                score += 0.4
                break
    ...
    def detect_relationships(
        self, data: Dict, parent_key: Optional[str] = None
    ) -> List[Dict]:
        """
                ...
                for key, value in obj.items():
                    ...
                    if key.endswith("_id") or key in {"id", "ref", "parent_id"}:
                        relationships.append(
                            {"source": parent, "target": key, "type": "key-based"}
                        )
...
def _extract_pagination_info(json_obj: dict) -> Dict:
    ...
    """
    pagination_info = {"type": None, "fields": [], "structure": {}}
...
def _analyze_response_structure(json_obj: dict) -> Dict:
    ...
    for key in error_keys:
        if key in json_obj:
            analysis["error_handling"] = {"location": key, "structure": json_obj[key]}
            break
